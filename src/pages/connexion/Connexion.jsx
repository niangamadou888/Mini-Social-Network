...
export default function Connexion() {
  ...
  const onSubmit = (data) => {
    axios
      .get(`https://mini-social-network.onrender.com/users/userExist/${data.mailUtilisateur}&${data.motDePasse}`)
      .then((res) => {
        if (res.data > 0) {
      ...
      <Box width={400} sx={{ backgroundColor: '#fff', padding: 3 }}>
        ...
        <form style={{ marginTop: 4 }} onSubmit={handleSubmit(onSubmit)}>
          ...
          <Typography paddingTop={2}>
            Voulez-vous créer un compte ? <Link onClick={inscription}>Cliquez ici</Link>
          </Typography>
        </form>
      </Box>
    </Stack>
